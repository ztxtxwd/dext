...
export function useConnection({
  ...
  const handleAuthError = async (error: unknown) => {
    if (is401Error(error)) {
      ...
      const serverAuthProvider = new DextOAuthClientProvider(sseUrl, scope);
  ...
  const connect = async (_e?: unknown, retryCount: number = 0) => {
    ...
    try {
      ...
      // proxying through the dext server first.
      const headers: HeadersInit = {};

      // Create an auth provider with the current server URL
      const serverAuthProvider = new DextOAuthClientProvider(sseUrl);
      ...
      try {
        ...
        console.error(
          connectionType === "direct"
            ...
            : `Failed to connect to MCP Server via the MCP Dext Proxy: ${serverUrl}:`,
          error,
        );
  ...
  const disconnect = async () => {
    ...
    const authProvider = new DextOAuthClientProvider(sseUrl);
    authProvider.clear();
    setMcpClient(null);
