...
import { DEFAULT_DEXT_CONFIG, CLIENT_IDENTITY } from "../../constants";
import {
    ...
    test("uses the default config values in makeRequest", async () => {
      ...
      expect(mockClient.request).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          timeout: DEFAULT_DEXT_CONFIG.MCP_SERVER_REQUEST_TIMEOUT.value,
          maxTotalTimeout:
            DEFAULT_DEXT_CONFIG.MCP_REQUEST_MAX_TOTAL_TIMEOUT.value,
          resetTimeoutOnProgress:
            DEFAULT_DEXT_CONFIG.MCP_REQUEST_TIMEOUT_RESET_ON_PROGRESS.value,
        }),
      );
    });
  ...
  describe("MCP_PROXY_FULL_ADDRESS Configuration", () => {
    ...
    test("sends proxyFullAddress query parameter for stdio transport when configured", async () => {
      const propsWithProxyFullAddress = {
        ...
        config: {
          ...
          MCP_PROXY_FULL_ADDRESS: {
            ...DEFAULT_DEXT_CONFIG.MCP_PROXY_FULL_ADDRESS,
            value: "https://example.com/dext/mcp_proxy",
          },
        },
      };
      ...
      expect(mockSSETransport.url?.searchParams.get("proxyFullAddress")).toBe(
        "https://example.com/dext/mcp_proxy",
      );
    });
